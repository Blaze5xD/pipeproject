...
int main(int argv,char **argc)
{
	...
	SDL_Window *win = SDL_CreateWindow("Game", 300, 300, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
	...
	while(!quit)
	{
		while (SDL_PollEvent(&event))
		{
			...
			else if(choose==1)
			{
				...
				{
					go_level_one(ren,DPIPES,DOUBPIPES,CROSSPIPES,a,back,choose);
				}
			}
			else if(choose==2)
			{
			...
			else if(choose==3)
			{
				...
				{
					go_level_two(ren,DPIPES,DOUBPIPES,CROSSPIPES,a,back,choose);
				}
			}
			else if(choose==4)
			{
			...
			else if(choose==5)
			{
				...
				{
					go_level_three(ren,DPIPES,DOUBPIPES,CROSSPIPES,a,back,choose);
				}
			}
		}
	}
