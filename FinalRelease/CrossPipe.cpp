...
{
	SDL_Texture *CrossPipe1 = IMG_LoadTexture (ren, hor.c_str());
	SDL_RenderCopy(ren, CrossPipe1, NULL, &Hor);
	SDL_Texture *CrossPipe2 = IMG_LoadTexture (ren, vert.c_str());
...
void nochangesCross(SDL_Renderer* ren,vector<CrossPipe>& a)
{
	...
	{
		a[i].default_create(ren,"CrossHor.jpg","CrossVert.jpg");
	}
}